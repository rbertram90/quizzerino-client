class ConnectForm {
    ...
    constructor(gameObject: Game, parentElement, lastConnection) {
    ...
    public generate() {
        ...
        let rememberMe = <HTMLInputElement> helper.checkbox({ id:'remember_me', parent:rememberMeWrapper, label:t('Remember these details') });
        if (remember_me) {
        ...
        let iconField = <HTMLInputElement> helper.element({ tag:'input', type:'hidden', name:'icon', value:default_icon, parent:iconWrapper });
    
        for (let i = 1; i <= 28; i++) {
            let icon = <HTMLImageElement> helper.element({ tag:'img', src:`/images/player-icons/${i}.png`, class:'player-icon',
