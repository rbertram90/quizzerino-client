...
import { DOMHelper, DOMHelperData, DOMHelperDivData } from "../domhelper";
import { Game } from "../game";
import { Message, MessageWithPlayers } from "../messages";
    ...
    public sendMessage(message: Message) {
        switch (message.type) {
            ...
            case "player_submitted":
                this.triggerRedraw(message as MessageWithPlayers);
                break;
        }
    }

    public triggerRedraw(message: MessageWithPlayers) {
        this.players = message.players;
        this.redraw();
    }

    public redraw() {
        let helper = this.game.service("domhelper") as DOMHelper;
        let cardWrapper = document.querySelector(".connected-players") as HTMLElement;

        if (! cardWrapper) {
            cardWrapper = helper.div({ class:"connected-players" } as DOMHelperDivData);
        ...
        helper.element({ tag:"h2", text:t("Players"), parent:cardWrapper } as DOMHelperData);
